...
public:
	...
	FontAsset(SDL_Renderer* c, std::string path, int pt_size, SDL_Color color);
	
	/// <param name="path">filename with font (*.ttf)</param>
	/// <param name="pt_size">what size (pt - points) font will be loaded.</param>
	/// <param name="chars">What characters will be loaded from font. Default = all</param>
	FontAsset(SDL_Renderer* c, std::string path, int pt_size, SDL_Color color, std::string chars);
	...
	SDL_Renderer* __getren() { return r; }
private:
